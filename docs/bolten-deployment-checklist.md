...
Antes do deploy, aplique no projeto Supabase o arquivo [20260827000000_bolten_sync_reliability.sql](../supabase/migrations/20260827000000_bolten_sync_reliability.sql). A verificação realizada nesta tarefa encontrou o schema remoto sem `bolten_sync_status` e sem `bolten_webhook_events`; portanto, essa migração ainda precisa ser executada no SQL Editor do Supabase.

Na Vercel, configure as variáveis privadas `BOLTEN_API_KEY`, `BOLTEN_CONTACT_COMPONENT_ID`, `BOLTEN_KANBAN_COMPONENT_ID`, `BOLTEN_WEBHOOK_SECRET` e `SUPABASE_SERVICE_ROLE_KEY`. Mantenha `VITE_SUPABASE_URL` e `VITE_SUPABASE_ANON_KEY` como as únicas variáveis públicas do navegador relacionadas ao banco. Não publique a chave Bolten com prefixo `VITE_`.
